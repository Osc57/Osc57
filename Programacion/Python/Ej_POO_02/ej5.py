...
class Password:
    ...
    def __init__(self,):
        self.longitud = 8
        self.contasena = self.genera_contrasena()

    def genera_contrasena(self):
        passw = ''
        for i in range(self.longitud):
            numero_o_letra = random.randint(0, 1)
            if numero_o_letra == 0:#sera un número
...
longitud_lista = int(input("Indica el número de password que quieres generar: "))

for i in range(longitud_lista):
    password = Password()
    password.longitud = longitud_password
    password.contasena = password.genera_contrasena()
    lista.append(password)

for i in range(len(lista)):
    passwd = lista[i]
    print(passwd.contasena + " " + str(passwd.es_fuerte()))
