...
CREATE TABLE VIDEOJUEGOS (
	...
	precio DECIMAL(6,2),
	id_genero INT,
	activo CHAR(1) DAFAULT 'S',
	
	CONSTRAINT ck_anio_lanzamiento CHECK (anio_lanzamiento BETWEEN 1970 AND YEAR(CURRENT_DATE) + 1),
...
CREATE TABLE JUGADORES (
	...
	monedas_virtuales DEFAULT 0,
	estado_cuenta VARCHAR(12) DEFAULT 'ACTIVA',
	
	CONSTRAINT ck_nivel_cuenta CHECK (nivel_cuenta > 0),
	CONSTRAINT ck_monedas_virtuales CHECK (nivel_cuenta >= 0),
...
CREATE TABLE PARTIDAS (
    ...
    CONSTRAINT fk_juego_parida FOREIGN KEY (cod_juego) REFERENCES VIDEOJUEGOS(cod_juego) ON DELETE RESTRICT
);
...
CREATE TABLE PARTICIPACIONES (
    ...
    CONSTRAINT ck_tiempo_jugado CHECK (tiempo_jugado_seg >= 0),
